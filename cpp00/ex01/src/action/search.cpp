...
/*   Updated: 2024/05/14 00:16:05 by babonnet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void printContact(Contact Contact, int index) {
    std::cout << std::setw(COLUMN_SIZE) << index << "|"
              << std::setw(COLUMN_SIZE) << shorten(Contact.getFirstName())
              << "|" << std::setw(COLUMN_SIZE) << shorten(Contact.getLastName())
              << "|" << std::setw(COLUMN_SIZE) << shorten(Contact.getNickname())
              << std::endl;
}
...
static void printInfo(Contact Contact) {
    std::cout << std::setw(16) << "FIRST NAME: " << Contact.getFirstName()
              << std::endl
...
static int fill_number(const char *str) {
    ...
    if ((*endptr && !isdigit(*endptr) && !isspace(*endptr)) || i < 0 ||
        i > INT_MAX)
        return (-1);
    if (endptr - str > 11)
        return (-1);
...
void PhoneBook::SEARCH(void) {
    ...
    if (index >= 8 || index < 0)
        std::cout << "this index " << index + 1 << " is out of scop"
                  << std::endl;
